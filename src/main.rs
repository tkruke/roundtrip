...
fn get_matrix_dimension() -> (usize, usize) {
    loop {
        ...
        stdin().read_line(&mut input_n).expect("Could not read line");
        ...
        print!("M: ");
        stdout().flush().ok().expect("Could not flush stdout");
        stdin().read_line(&mut input_m).expect("Could not read line");
...
fn validate_board_size(n: usize, m: usize) -> bool  {
    if n > N_MAX || m > M_MAX {
        println!("n and m must be less or equal to {}", N_MAX);
    }
    if n > m {  // enforce that N <= M (an NxM matrix have same solutions as a MxN matrix, so this is just to be able to assert a 'high and thin' matrix when checking and printing solutions)
...
fn initialize_board(mut b: [[bool;N_MAX*M_MAX];N_MAX*M_MAX], n: usize, m: usize) -> [[bool;N_MAX*M_MAX];N_MAX*M_MAX] {
...
fn check_board(mut b:[[bool;N_MAX*M_MAX];N_MAX*M_MAX], mut visited: usize, mut solutions: i64, v: usize, n: usize, m: usize) -> i64 {
    ...
    b[v][v] = true;  // mark vertice v as visited
    visited += 1;
    if visited == n*m { //all vertices visited - can we make it back to the start vertice (0)? 
        if b[0][v] {  // success!
    ...
    for i in 0..n*m {
        if i != v && b[i][v] && !b[i][i] {                     // (i==v is no edge) there is an edge from v to i, and vertice i has not been visited yet
            solutions = check_board(b, visited, solutions, i, n, m);    // try finding a solution by traversing the edge from v to i and search for solutions from there
        }
    }
    return solutions;  
}
...
fn main() {
    loop {
        ...
        if validate_board_size(n, m) == false {
            ...
            let mut b = [[false; N_MAX*M_MAX]; N_MAX*M_MAX];
            b = initialize_board(b, n, m);
            println!("Searching solutions for {:?} x {:?} matrix", n, m);
            let run_duration = SystemTime::now();
            let mut solutions: i64 = 0; 
            b[0][0] = true;                 // start top left in vertice 0 
            let visited = 1;
            let vertice_to_visit = 1;  // define first step to the right to avoid checking solutions 'in both directions'
            solutions = check_board(b, visited, solutions, vertice_to_visit, n, m); // parameters: board, #visited_so_far, #solutions_so_far, vertice_to_visit, matrix_dimension_n, matrix_dimension_m
